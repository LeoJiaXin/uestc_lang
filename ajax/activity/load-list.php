<?php
  ...
  define(MAX_NUMBER_OF_ACTIVITIES_IN_A_PAGE,25);
  $list = array();
  $page_index = $_GET['page_pos']?$_GET['page_pos']:0;
  $sec_type = $_GET['sec_type'];
  if ($sec_type) {
    $args = array(
      'numberposts'     => MAX_NUMBER_OF_ACTIVITIES_IN_A_PAGE,
      'offset'           => $page_index*MAX_NUMBER_OF_ACTIVITIES_IN_A_PAGE,
      'category'    => $sec_type,
      'orderby'          => 'post_date',
      'order'            => 'DESC',
      'post_type'        => 'post',
    ...
    if ($employ_list) {
      for ($i=0;$i<count($employ_list);$i++) {
        ...
        $element->watched = getPostViews($element->id);
        unset($element->img);
    ...
    if ($page_index == 0) {
      ...
      if ($tmp && $tmp->count>0) {
        $list[0]->sum = ceil($tmp->count/MAX_NUMBER_OF_ACTIVITIES_IN_A_PAGE);
      }
    }
  }
